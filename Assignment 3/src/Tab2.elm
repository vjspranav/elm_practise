    ...
    case msg of
        ...
        Next ->        
            if model.size > List.length model.array || model.mu1_merge == False || model.mu2_merge == False then
                ...
                { model | 
                 ...
                 i = model.i, j = model.i + (model.size * 2), size = model.size, change = False, 
                 mu1_merge = if (List.length (getSubList model.i ((model.size * 2) + model.i) model.array) == 0) then True else False, 
                 mu1_sort = if (List.length (getSubList model.i ((model.size * 2) + model.i) model.array) == 0) then True else False, 
                 mu2_merge = if (List.length (getSubList (model.i + (model.size * 2)) ((model.size * 2) + (model.i + (model.size * 2))) model.array) == 0) then True else False, 
                 mu2_sort = if (List.length (getSubList (model.i + (model.size * 2)) ((model.size * 2) + (model.i + (model.size * 2))) model.array) == 0) then True else False,
        ...
        Sort2 ->
            if model.mu2_sort then
                ...
                {model | mergeUnit2 = (mergeSubList (getSubList 1 (model.size + 1) model.mergeUnit2) (getSubList (model.size+1) ((model.size*2) + 1) model.mergeUnit2)), mu2_sort = True, mu2_merge = False,
        ...
        Merge2 ->
            if model.mu2_merge then
                ...
                {model | array = (updateArray model.array model.mergeUnit2 model.j ((List.length model.mergeUnit2) + (model.j - 1))), i = model.i + (model.size * 2), mergeUnit2 = [], mu2_merge = True, mu2_sort = True,
